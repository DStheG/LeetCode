class Solution:
    def __init__(self):
        self.result = 0
        self.cache = [[-1 for _ in range(201)] for _ in range(101)]

    def recurSol(self, locations: list[int], cur: int, dest: int, fuel: int) -> int:
        print(cur, fuel)
        if (self.cache[cur][fuel] != -1):
            self.result += self.cache[cur][fuel]
            return 

        if (cur == dest):
            self.result += 1
            self.cache[cur][fuel] = 1
        else:
            self.cache[cur][fuel] = 0
        ...
        for i in range(len(locations)):
            if (i != cur):
                ...
                if (newFuel >= 0):
                    ...
                    self.cache[cur][fuel] += self.result - t
